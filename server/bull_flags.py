...
def check_flag(pending: FlagPattern, data: np.array, i:int):
    ...
    if data[pending.tip_x + 1 : i].max() > pending.tip_y: #check if a point in the flag is above the tip
       return False

    flag_min = data[pending.tip_x:i].min() #finds current minimum of the flag

    pole_height = pending.tip_y - pending.base_y #finds height of pole
    pole_width = pending.tip_x - pending.base_x #finds width of pole
    
    flag_height = pending.tip_y - flag_min #finds height of flag from current index
...
def find_bull_flags(data: np.array, order:int):
    ...
    bull_flags = [] #stores all the flags found, to be returned at the end of the function

    for i in range(len(data)):
        # Pattern data is organized like so:
        if check_top(data, i, order): #if a top is found at the current index
            last_top = i - order #sets last_top to the index of the top found
            if last_bottom != -1: #if there is also a last bottom
                ...
                pending.tip_x = last_top #sets start of the flag to the last top
                pending.tip_y = data[last_top]#sets start of the flag y value to last top value

                pending_flag = pending #sets pending flag to the flag created
        
        if check_bottom(data, i, order): #if bottom found at current index
            last_bottom = i - order #sets last_bottom to index of bottom found
        ...
        if pending_flag is not None: #if there is a pending flag
            if check_flag(pending_flag, data, i) and not pending_flag.pennant: #checks of flag is confirmed, and it is not a
                bull_flags.append(pending_flag.conf_x) #if so, flagpattern is added to bull_flags
